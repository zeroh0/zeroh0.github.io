...
public class BodyFatMethod {
	...
	static double getLeanBodyMass(String gender, double weight, double height) {
		...
		switch(gender) {
		case "M": leanBodyMass = (maleSTD*weight) - (COMSTD*((weight*weight) / (height*height))); break;
		case "F": leanBodyMass = (femaleSTD*weight) - (COMSTD*((weight*weight) / (height*height))); break;
		}
		return leanBodyMass;
	}
	...
	// 출력
	static void printResult(double leanBodyMass, double leanBodyRatio, double bodyFatMass, double bodyFatRatio, String result) {
	...
	static void isRun() {
		...
		if(yesNo.equalsIgnoreCase("no")) run = !run;
	}
